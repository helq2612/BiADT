...
def plot_raw_img_with_score(img, boxes, img_shape, labels, probs):
    """[summary]
    ...
    H, W = img_shape
    for box, label, score in zip(boxes.tolist(), labels.tolist(), probs.tolist()):
        x, y, w, h = box[0] * W, box[1] * H, box[2] * W, box[3] * H
        # import ipdb; ipdb.set_trace()
        img = cv2.rectangle(img.copy(), (int(x-w/2), int(y-h/2)), (int(x+w/2), int(y+h/2)), _color_getter(label), 2)
        # add text
        brand = "{},{:.2f}".format(label,score)
        org = (int(x-w/2), int(y+h/2))
        font = cv2.FONT_HERSHEY_SIMPLEX
        fontScale = 1
        thickness = 1
