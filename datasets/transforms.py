...
class StrongAug(object):
    ...
    def __call__(self, image, target=None):
        # print("check data type",  type(image))
        # exit(0)
        # image = image.numpy()
        # image_pil = np.array(image)
        # image_pil = Image.fromarray(image_pil.astype("uint8"), "RGB")
        # image_pil = image
        image_pil = self.strong_augmentation(image)
        # image_strong_aug = np.array(self.strong_augmentation(image_pil))
        # image = np.ascontiguousarray(image_strong_aug)
...
def build_strong_augmentation():
    ...
    if True:
        ...
        augmentation.append(transforms.ToPILImage())
        augmentation.append(
