...
from util.vis_utils import plot_dual_img, plot_raw_img_with_score
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        #=============================================prepare data ===========================================================
        all_samples_wa_wa, all_samples_wa_sa, all_samples_sa_wa, all_samples_sa_sa, wa_target_imgs, wa_1src_gts, wa_1tgt_gts = samples
        if args.training_phase == "wa_src_tgt":     # weak augmented images, one from source, one from target
            samples = all_samples_wa_wa.to(device)
            targets = [{k: v.to(device) for k, v in t.items()} for t in targets]    # targts is the wa_1src_1tgt_gts in util/misc.py
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            if need_tgt_for_training:
                outputs, mask_dict = model(samples, dn_args=(targets, args.scalar, args.label_noise_scale,
                                                             args.box_noise_scale, args.num_patterns), domain_flag=domain_flag, use_pseudo_label=use_pseudo_label)
                loss_dict = criterion(outputs, targets, mask_dict)
            else:
                outputs,_ = model(samples, domain_flag=domain_flag, use_pseudo_label=use_pseudo_label)
                loss_dict = criterion(outputs, targets, mask_dict={})
