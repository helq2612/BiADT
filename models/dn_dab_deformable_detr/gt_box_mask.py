...
def gt_box_mask_generate2(heatmap, boxes, categories, indices):
    """
    ...
    for i, bid in enumerate(indices):
        ...
        # heatmap[bid, category, top:bot, left:right] = 1

    heatmap = heatmap.permute(0, 2, 3, 1)
    heatmap[:, : , :, -1] = (heatmap[:, :, :, :-1].sum(dim=-1, keepdim=False)).eq(0)
    return heatmap


def gt_box_mask_generate_old(heatmap, boxes, categories, indices):
    """
    generate mask for boxes;
    TODO: will use grid sample next;
    TODO: will use gaussian heat map next;
